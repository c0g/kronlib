...
{
    TBBMatrix<float> mat(3, 3);
    mat = 1, 1, 0.5, 1, 10, 3.5, 0.5, 3.5, 2.25;
    Cholesky<TBBMatrix<float>> chol(mat);
    TBBMatrix<float> mat_ans(3, 3);
    mat_ans = 1,   0, 0, 
	          1,   3, 0, 
              0.5, 1, 1;
...
TEST(Cholesky, Solve)
{
    ...
    TBBMatrix<float> solve_with(3, 4);
    solve_with = 1, 1, 1, 	
		1, 2, 3, 
		1, 3, 5,
		0, 3, 6;

    TBBMatrix<float> ans(3, 4);
    ...
    TBBMatrix<float> solved = chol.solve(solve_with);
...
{
    TBBMatrix<double> mat(4, 4);
    mat = 125,  195,  275,  429,  195,  305,  429,  671,  275,  429,  625,
        975,  429,  671,  975, 1525;
    Cholesky<TBBMatrix<double>> chol(mat);

    TBBMatrix<double> solve_with(4, 1);
    solve_with = 0, 1, 2, 3;

    TBBMatrix<double> ans(4, 1);
