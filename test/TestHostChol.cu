...
{
    HostMatrix<float> mat(3, 3);
    mat = 1, 1, 0.5, 1, 10, 3.5, 0.5, 3.5, 2.25;
    Cholesky<HostMatrix<float>> chol(mat);
    HostMatrix<float> mat_ans(3, 3);
    mat_ans = 1, 0, 0, 
	      1, 3, 0, 
              0.5, 1, 1;
    EXPECT_TRUE(mat_ans == chol.cholmat());
}
/*
...
TEST(Cholesky, Solve)
{
    ...
    HostMatrix<float> solve_with(3, 4);
    solve_with = 1, 1, 1, 	
		1, 2, 3, 
		1, 3, 5,
		0, 3, 6;

    HostMatrix<float> ans(3, 4);
    ...
    HostMatrix<float> solved = chol.solve(solve_with);
...
{
    HostMatrix<double> mat(4, 4);
    mat = 125,  195,  275,  429,  195,  305,  429,  671,  275,  429,  625,
        975,  429,  671,  975, 1525;
    Cholesky<HostMatrix<double>> chol(mat);

    HostMatrix<double> solve_with(4, 1);
    solve_with = 0, 1, 2, 3;

    HostMatrix<double> ans(4, 1);
